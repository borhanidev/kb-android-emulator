...
export function GpuSettings({ toast, gpus, hypervisor, sysInfo, loading, onRescan }) {
  ...
  const handleQuickBootToggle = () => {
    ...
    toast(val ? '⚡ Quick Boot (Snapshots) enabled' : '❄️ Cold Boot mode enabled (starts fresh)', 'success')
  }
  ...
  const applyGpuPreference = async () => {
    ...
    toast(ok
      ...
      : `⚠️ ${result?.error || 'Some registry entries may need administrator rights.'}`,
      ok ? 'success' : 'warn')
  }
  ...
  const enableWhpx = async () => {
    ...
    toast(result.ok ? '✅ WHPX enabled — please restart your PC.' : '❌ Failed. Try running as administrator.', result.ok ? 'success' : 'error')
  }
  ...
  const GPU_PREFS = [
    ...
    { val: 0, label: '⚙️ System Default', desc: 'Let Windows decide which GPU to use' },
  ]
  ...
  const ACCEL_OPTIONS = [
    ...
    { val: 'aehd', label: '🛡️ AEHD (Android Emulator Hypervisor Driver)', desc: 'Google\'s lightweight standalone driver. Works on Intel/AMD. (Must disable Hyper-V to run AEHD).' },
    { val: 'haxm', label: '⚙️ Intel HAXM (Legacy)', desc: 'Intel\'s hardware acceleration driver. Works on Intel CPUs only (Deprecated).' },
    { val: 'off',  label: '🖥️ Off (No Virtualization)', desc: 'Extremely slow! Emulates CPU in software. Use only for diagnostic troubleshooting.' },
  ]
  ...
  const GPU_MODES = [
    ...
    { val: 'swiftshader_indirect', label: '🖥️ SwiftShader (Software)', desc: 'CPU-rendered graphics. Slow, compatibility mode only.' },
  ]
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      {sysInfo && (
        <div className="grid-3">
          {[
            { icon: '💻', label: 'CPU', val: sysInfo.cpu_model?.split('@')[0]?.trim(), sub: `${sysInfo.cpu_count} threads` },
            { icon: '🧠', label: 'Total RAM', val: `${Math.round(sysInfo.total_ram / 1024)} GB`, sub: `${Math.round(sysInfo.free_ram / 1024)} GB free` },
            { icon: '🖥️', label: 'Architecture', val: sysInfo.arch, sub: sysInfo.platform },
          ].map(s => (
      ...
      {/* Virtualization Acceleration Options */}
      <div className="card">
        <div className="card-title">⚡ Virtualization Accelerator (accel)</div>
        ...
        {selectedAccel === 'whpx' && !hypervisor?.whpx_enabled && (
          <div className="alert alert-warn mt-3" style={{ display: 'flex', justifyContent: 'between', alignItems: 'center' }}>
            <span>⚠️ WHPX is selected but not enabled in Windows.</span>
      ...
      <div className="card">
        <div className="card-title">🎮 Launch GPU Rendering Mode (gpu)</div>
        <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 16 }}>
      ...
      {/* Launch & Boot Options */}
      <div className="card">
        <div className="card-title">🚀 Launch & Boot Options</div>
        <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div className="flex items-center justify-between" style={{ padding: '4px 0' }}>
            <div style={{ flex: 1, paddingRight: 16 }}>
              <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 2 }}>⚡ Quick Boot (Snapshot Resume)</div>
          ...
          <div className="flex items-center justify-between" style={{ padding: '4px 0' }}>
            <div style={{ flex: 1, paddingRight: 16 }}>
              <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 2 }}>🎬 Show Android Loading Logo</div>
      ...
      <div className="card">
        <div className="card-title">⚡ Speed & Performance Tweaks</div>
        <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div className="flex items-center justify-between" style={{ padding: '4px 0' }}>
            <div style={{ flex: 1, paddingRight: 16 }}>
              <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 2 }}>📷 Disable Webcam & Camera Emulation</div>
          ...
          <div className="flex items-center justify-between" style={{ padding: '4px 0' }}>
            <div style={{ flex: 1, paddingRight: 16 }}>
              <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 2 }}>📍 Disable Location / GPS Polling</div>
          ...
          <div className="flex items-center justify-between" style={{ padding: '4px 0' }}>
            <div style={{ flex: 1, paddingRight: 16 }}>
              <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 2 }}>📡 Disable Bluetooth Simulation</div>
      ...
      {/* Windows GPU Preference Selection */}
      <div className="card">
        <div className="card-title">🔌 Windows GPU Preference (Registry Override)</div>
        <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 16 }}>
        ...
        <div className="alert alert-info" style={{ marginBottom: 16 }}>
          💡 Written to <span className="font-mono" style={{ fontSize: 11 }}>HKCU\Software\Microsoft\DirectX\UserGpuPreferences</span> — same as Windows <strong>Settings → Display → Graphics Settings</strong>.
        </div>

        <button className="btn btn-primary" onClick={applyGpuPreference} disabled={applying || selectedGpu === null}>
          {applying ? <><Spinner size={15} />Applying…</> : '🎮 Apply Windows GPU Registry'}
        </button>
      </div>

      <div className="card">
        <div className="card-title">🚀 Performance Tips</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {[
            { icon: '🎮', tip: 'Use GPU mode "Host" when creating a device — passes graphics directly to your GPU with zero translation overhead.' },
            { icon: '📦', tip: 'Always use x86_64 system images — no ARM emulation overhead, runs at native CPU speed.' },
            { icon: '🧠', tip: 'Allocate at least 4 CPU cores and 4 GB RAM for gaming apps.' },
            { icon: '⚡', tip: 'Enable WHPX (Windows Hypervisor Platform) for CPU virtualization — massive speed improvement.' },
            { icon: '💾', tip: 'Use fast boot (snapshot) to avoid waiting for Android to cold boot each time.' },
            { icon: '🔋', tip: 'Close other GPU-heavy apps while gaming in the emulator for best frame rates.' },
          ].map((t, i) => (
